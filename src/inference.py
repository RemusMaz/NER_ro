...
def predict(model, tokenized_input_text):
    ...
    for i in range(len(tokenized_input_text)):
        token_ids = model.tokenizer.encode(tokenized_input_text[i],
                                           add_special_tokens=False)  # tokenize the word, CAREFUL as it could give you 2 or more tokens per word
        input_ids.extend(token_ids)
        token_idx.extend([i] * len(token_ids))  # save for each added token_id the same word position i
    ...
    prediction_int = torch.argmax(logits,
                                  dim=-1).squeeze().tolist()  # reduce to [seq_len] as list, as batch_size = 1 due to .squeeze()
